...
> **Status:** living master plan, **revision 8 + S3 execution-plan addendum (2026-07-15), S3.b2 partial-scope decision (2026-07-21)**. Save schema **v8**; transactions **Phases 1–5b implemented**; Stage 1 accepted; F1 done; **S2a through S2d landed**; **F2 accepted and merged 2026-07-15**; **S3.a accepted 2026-07-15**; and **S3.b1 accepted 2026-07-18** with its supply-adjusted matchup lift, fixed one-draw selector, and activated-pool profile PASS 32/32 (evidence in `docs/PROJECT_STATUS.md`). A read-only S3.b2 diagnostic authorized a partial zone-influence implementation while preserving long-midrange's perimeter-only legacy behavior; runtime work has not started. The NBA-derived pool/selector/diets remain the sole production path; **partial S3.b2 is the next simulation unit and F3 is the next independent franchise unit**. Every claim below was verified against source or execution — where documents disagreed, the code won (§1.4).
>
...
| 3 | **F2 — playoffs** ✅ merged 2026-07-15 ∥ **S3 — Stage 3 mechanics (first tranche)** | F / S | **F2 accepted and merged; S3.a and S3.b1 accepted. Partial S3.b2 is authorized and next; runtime remains unimplemented.** Continue landing one unit at a time on main; F2 makes the championship metric real before the 5c baseline. |
...
| **Defender-zone influence mapping** | prospective generated block in `src/engine/constants.ts` + `docs/S3B2_DEFENDER_INFLUENCE.md` | `node --import tsx scripts/derive-s3b2-defender-influence.ts` (`--check` will byte-compare both outputs) | **S3.b2 partial scope authorized; not implemented.** Full-window derived rim, short-midrange, and shared-3PT weights. `long_midrange` remains a separately named perimeter-only legacy fallback (`interiorWeight = 0`) because long two's unchanged 95% slope-sum gate includes zero in the full and both half-windows; it is measured, not identified. Decision diagnostic SHA-256 `c147250bf80c745f700947f92f1585643d8d2cae6ea0fb00269cdd7b47cc448d`. |
