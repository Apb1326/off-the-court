...
> **Status:** living master plan, **revision 7 (2026-07-06)**. Grounded in the live repository and the S1-R acceptance runs of 2026-07-06 (save schema **v6**, transactions **Phases 1–5b implemented**, NBA data pipeline **Stage 0 built, OP-1 harvest RUN, Stage 1 IMPLEMENTED AND ACCEPTED after the S1-R repair**). Every claim below was verified against source or execution — where documents disagreed, the code won (§1.4).
>
...
**The working pattern** (how execution units get built here): the feature is discussed and agreed first; a complete, self-contained Claude Code prompt is produced (no external context assumed — Claude Code has no cross-session memory); the prompt is sanity-checked on two axes (engineering soundness, basketball/architecture domain logic) and every finding is folded into a single final version; the final prompt is committed under `docs/prompts/` before it runs (the folder exists as of R0b; the legacy `PHASE_*_IMPLEMENTATION_PROMPT.md` files live there now, alongside the archived Stage 1 prompt). §10 contains the per-unit prompt skeleton to instantiate.
...
- **The assisted sign-structure failure** — DIAGNOSED (S1-Rb): independent of the margin cause; routing is correct, the gap is the definitional mismatch between the NBA scorekeeper assist and the engine's strict chain assist. Handed to the S2/S3 assist-definition mapping owner (§4.1a outcome).
...
- **Track S — Simulation & data** (§4): the pipeline-driven engine overhaul. Stage 1 (implemented and accepted; **S1-R repair landed**) → Stage 2 (ratings/tendencies derivation from data; retire BallDontLie; unwind the Stage-1 shot-diet shading) → Stage 3 (mechanics enabled by richer data). Strictly sequential within the track.
...
|---|---|---|---|---|---|---|---|---|
| **S1** *(implemented + accepted)* | ● done (targets derived + transcribed) | ✅ **PASSING** (margin repaired by S1-R) | ○ (report deltas) | ○ | ○ (green in local run) | ○ (0.781 nit — §1.3.11) | n/a yet | n/a yet |
...
**Residuals (owners assigned):** margin acceptance failure + assisted sign-structure diagnosis + engine-seed boundary fix → **S1-R** (§4.1a — ✅ all three delivered); the `PLAY_TYPE_SHOT_ZONES` shading unwind → **S2**; the assist-definition mapping → **S2/S3** (now carrying S1-Rb's written diagnosis); the FT anchor nit → **S2**; the play-type frequency skew → **S2**.
...
#### S2a/S2b implementation addendum — 2026-07-09

S2a and S2b are implemented. Repair unit **S2b-R** corrected the two original deviations from this section's derivation table: `box_advanced.per100.tov` was replaced with the usage-normalized `box_advanced.advanced.tmTovPct`, and the shared `defense.defended.overall` signal was split into category-specific perimeter/interior inputs. The candidate remains inactive pending S2c/S2d.

The normalize defect was a per-category `LeagueDashPtDefend` column rename mismatch: `build_defense` read the Overall columns for every category, creating null-scaffolded category lines. Validation missed this because nullable fields were accepted without a population-level scaffold check. Commit 1 added explicit per-category mappings, reran normalization from the existing 2013–2025 raw cache without re-harvesting, and added a validator gate requiring finite `dFga` for at least 95% of any category with more than 25 rows. The discovered mapping is: `Overall → {dFgm:D_FGM, dFga:D_FGA, dFgPct:D_FG_PCT, normalFgPct:NORMAL_FG_PCT, pctPlusMinus:PCT_PLUSMINUS}`; `3 Pointers → {dFgm:FG3M, dFga:FG3A, dFgPct:FG3_PCT, normalFgPct:NS_FG3_PCT, pctPlusMinus:PLUSMINUS}`; `2 Pointers → {dFgm:FG2M, dFga:FG2A, dFgPct:FG2_PCT, normalFgPct:NS_FG2_PCT, pctPlusMinus:PLUSMINUS}`; `Less Than 6Ft → {dFgm:FGM_LT_06, dFga:FGA_LT_06, dFgPct:LT_06_PCT, normalFgPct:NS_LT_06_PCT, pctPlusMinus:PLUSMINUS}`; `Less Than 10Ft → {dFgm:FGM_LT_10, dFga:FGA_LT_10, dFgPct:LT_10_PCT, normalFgPct:NS_LT_10_PCT, pctPlusMinus:PLUSMINUS}`; `Greater Than 15Ft → {dFgm:FGM_GT_15, dFga:FGA_GT_15, dFgPct:GT_15_PCT, normalFgPct:NS_GT_15_PCT, pctPlusMinus:PLUSMINUS}`. The repaired rotation-level coverage is 270/270 for both `threePointers` and `lessThan6Ft`; the 2013–2025 raw pt_defend window remains an available multi-season option for the S2d/S3.a owner of single-season defense noise, but S2b consumes only 2025–26.

S2b-R also records the D4 stamina/durability split (mpg-only vs. gp-only; DNP-CD/role conflation remains owned by F4/Horizon), D5 ratings-consistent contracts and rotations, D7's unchanged truncated-normal tail policy with star-separation behavioral evaluation assigned to S2d, and D8's unchanged active-pool compatibility SD priors with the `Math.max(4, active-pool SD)` floor. S2d inherits the tail/star-separation behavioral evaluation, single-season defense-data noise, the avgSpeed-as-athleticism limitation, and the durability DNP-CD conflation.
...
| A/B expectations | `scripts/test-spacing-ab.ts`, `test-defense-ab.ts` | Re-verified at S1-R (green on the repaired engine); re-verify again at S2 | Material, correctly-signed effects. Green 2026-07-06. |
