...
> **Status:** living master plan, **revision 8 (2026-07-10)**, verified at `29ad959`. Save schema **v7**; transactions **Phases 1–5b implemented**; Stage 1 accepted; F1 done; and **S2a, S2b, and S2b-R landed**. The pipeline-derived candidate league remains inactive: **S2c1 is next**, F2 is independently dependency-ready, and S3 remains gated on S2d. Every claim below was verified against source or execution — where documents disagreed, the code won (§1.4).
>
...
- **Track S — Simulation & data** (§4): the pipeline-driven engine overhaul. Stage 1 (implemented and accepted; **S1-R repair landed**) → Stage 2 (S2a/S2b done on an inactive candidate; S2c1/S2c2/S2d remain) → Stage 3 (mechanics enabled by richer data). Strictly sequential within the track.
...
| 2b | **S2c1 — candidate tendencies + evaluation seam** | S | **Implemented.** Candidate-only real usage/play-type/shot-mix derivation, coverage contract, and read-only informational profiling seam landed; candidate remains inactive. |
| 2c | **S2c2 — assist decision + compensation unwind** | S | After S2c1. Settle the S1-R assist-definition hand-off and remove the documented Stage-1 shot-diet shading on the candidate. |
| 2d | **S2d — activation, re-baseline, legacy retirement** | S | After S2c2. Sole activation point for the candidate; re-derive baselines and re-pass profile. |
...
3. **S2c1 — tendencies + candidate evaluation seam.** Derive real usage/play-type/shot-mix tendencies in the candidate only, including its coverage/fallback report. Add an explicit read-only `--league-dir` (or equivalent) input to the profiling tools so the candidate can be simulated and its informational tables reported without copying over `data/players.json` or `data/teams.json`. Default invocations continue to use the active pool, and active-pool profile/calibrate output remains byte-identical.
4. **S2c2 — assist-definition decision + compensation unwind.** On the candidate, decide and document the mapping between the NBA scorekeeper assist and the strict chain assist, then return `PLAY_TYPE_SHOT_ZONES` toward its real per-type diets and remove the named Stage-1 shading. A decision to leave engine assist-credit mechanics to S3.g is valid, but must be explicit; a silent non-use of the per-zone evidence is not.
5. **S2d — activation, coupled re-baseline, and legacy-ingest retirement.** This is the **sole activation point**: atomically promote the accepted candidate to the active new-league path, re-derive spacing/versatility baselines, settle the FT inverse pair, retune the realized profile to PASS, then delete or archive the superseded BDL generation path and resolve `scripts/seed-test.ts`.
...
**The Stage-1 compensation unwind (S2c2's named duty):** once S2c1's derived candidate tendencies bring the league play-type distribution near the Synergy reference, **re-tune `PLAY_TYPE_SHOT_ZONES` back toward the real per-type shot diets** noted in its own comments, and remove or rewrite the `KNOWN STAGE 2 ARTIFACT` block. Evidence the unwind actually happened: the candidate informational play-type table moving into the vicinity of the Synergy shares while its zone shares remain reportable. If the distribution can't get close enough for real diets to hold the zone shares in band, that's a stop-and-surface (it would mean play-type *selection* mechanics are the binding constraint — a Stage 3/Horizon question).
