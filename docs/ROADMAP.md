...
> **Status:** living master plan, **revision 6 (2026-07-06)**. Grounded in the live repository and the maintainer's local acceptance run of 2026-07-06 (save schema **v6**, transactions **Phases 1–5b implemented**, NBA data pipeline **Stage 0 built, OP-1 harvest RUN, Stage 1 IMPLEMENTED — ACCEPTANCE FAILING**). Every claim below was verified against source or execution — where documents disagreed, the code won (§1.4).
>
...
**The working pattern** (how execution units get built here): the feature is discussed and agreed first; a complete, self-contained Claude Code prompt is produced (no external context assumed — Claude Code has no cross-session memory); the prompt is sanity-checked on two axes (engineering soundness, basketball/architecture domain logic) and every finding is folded into a single final version; the final prompt is committed under `docs/prompts/` before it runs (the existing `docs/PHASE_*_IMPLEMENTATION_PROMPT.md` files are this convention's ancestors — going forward, use a `docs/prompts/` folder). §10 contains the per-unit prompt skeleton to instantiate.
...
### 1.0 The local acceptance run (2026-07-06) — what it settled

The maintainer executed the verification suite on the working machine, resolving what the snapshot alone could not:
...
- **Six-zone shot model (Stage 1).** `ShotZone` in `models/game.ts` is `rim | short_midrange | long_midrange | corner_three | above_break_three | deep_three`, with the settled NBA-shot-chart mapping documented in `constants.ts` and `docs/LEAGUE_TARGETS.md`: rim = Restricted Area only; short_midrange = Paint (Non-RA) + Mid-Range < 14 ft; long_midrange = Mid-Range ≥ 14 ft; deep_three = above-the-break threes ≥ 27 ft (below the 32 ft heave cutoff). Zone bases (`BASE_FG_PCT_BY_ZONE`) and `PLAY_TYPE_SHOT_ZONES` are retuned against the empirical targets with in-file provenance annotations. The old five-vs-six zone-taxonomy drift is **resolved** — but note the retune has not fully passed acceptance (§1.0).
- Spacing & defensive versatility (`spacing.ts`): centered off-ball-four spacing (Moreyball shot-mix hooks, rim-deterrence relief, openness), weak-link versatility floor with mobility/size spread penalties, both pure arithmetic (no RNG). Baselines are static constants (`SPACING_BASELINE_OFFBALL_FOUR = 0.3171`, `SPACING_SPREAD = 0.0622`, `VERSATILITY_BASELINE = 0.6789`, `VERSATILITY_SPREAD = 0.0928`) derived by `tsx scripts/calibrate-spacing.ts`; the drift limitation of a static baseline is documented in `constants.ts` and deliberately deferred (picked up in **F4**, §5.4). Both A/B tests are **green in the local run**.
...
- Game loop (`index.ts`): momentum, home-court edge, clamped per-game form (±13%), forced mid-game exits for in-game injuries (`inGameExits`), stats derived exclusively from the `PlayByPlayEvent` stream via `recordEventStats` → `StatsAccumulator`. The `addXStats` stubs remain intentional no-ops. *(The margin failure most plausibly lives among these game-flow mechanisms — see S1-R's diagnostic scope, §4.1a.)*
- Tactics (`tactics.ts`): clock management, three-chasing, intentional fouls. Shot resolution (`shot.ts`) is additive-and-clamped per `AGENTS.md`, with the FT inverse pair (`FT_LEAGUE_AVG_PCT = 0.781` / `FT_PCT_SLOPE` / `FT_DERIVE_SCALE`, sim clamps `FT_SIM_PCT_MIN/MAX`). *Residual note:* the empirical enforced FT% target is **0.7823**; `FT_LEAGUE_AVG_PCT` sits at 0.781 — inside the 0.01 tolerance band, but not literally re-anchored. Resolve deliberately at S2's round-trip re-assertion.
...
- Season advancement (`season.ts`): monotonic and idempotent (`currentDate` never rewinds; completed game IDs never re-simulate); per-game seeds derive from `season.seed + gameId`, so "full RNG state" is exactly `season.seed` + the set of played games. **However: `createSeasonState`'s seed default calls `Math.random()` inside `src/engine`** — a live violation of the determinism rule as written in `AGENTS.md` (which prohibits `Math.random` in the engine without exception). Revision 3 papered over this with a policy exception; revision 4 repeals that: the fix is architectural (seed required by the engine; ambient selection moves to the menu/API boundary), executed in **S1-R** (§4.1a).

**Stage 1 calibration instrumentation (implemented — acceptance failing; the full record is §4.1):**
...
- `SAVE_SCHEMA_VERSION = 6` with an ordered migration chain: v1→v2 (FA pool + transaction log), v2→v3 (full contracts, generated deterministically from `SeededRNG(fnv1a(player.id))` at the shared `normalizePlayersForSave` boundary), v3→v4 (explicit re-signing rights reconstructed from immutable cut snapshots; persisted event-set hard-cap state), v4→v5 (TPE grants + operated-under-cap ledgers, empty-init), v5→v6 (recompute persisted usage + free-throw derivations from canonical career stats). `scripts/recompute-derived.ts` is the standalone refresh tool; `scripts/test-save-migration.ts` and `scripts/test-contract-migration.ts` are the round-trip checks.
...
- The gate threads an optional `controlledTeamId` through trade options for NTC consent (`noControlledTeamNtc`), but **no canonical controlled-team identity exists anywhere in `SeasonState`/`SaveFile`**. (Fixed by **F1**, §5.1.)
...
**App shell** (`src/app/`): menu (new game / save management), league, roster, schedule (with standings), player detail, single-game sim page; API routes for players, teams, season, sim, saves. No transaction UI, no playoffs, no offseason flow, no controlled-team concept in the UI.
...
1. **S1 is implemented but its acceptance is FAILING (revision 4 — reopened).** As of the 2026-07-06 local run: average margin 16.8 vs 12.87 ± 1.0 (ENFORCED — the profile exits non-zero), and the informational assisted-zone sign-structure check fails (corner three not highest-assisted in engine output). Everything else in the run is green. **S1-R** (§4.1a) owns the repair; no phase that depends on a green engine baseline starts before it.
2. **Engine constructors still choose ambient seeds:** `createSeasonState` falls back to `Math.random()` and `simulateSeason` falls back to `Date.now()` inside `src/engine/season.ts` — live violations of the engine determinism rule as written. Revision 3's policy exception is **repealed**; both constructors require seeds, while the new-season API validates or chooses one at the app boundary. Executed in **S1-R**. The remaining `Math.random` ledger: `src/data/ingest/transforms.ts:77` (BDL placeholder contract years — real violation, mooted when S2 retires the BDL path); `scripts/seed-test.ts` throughout (out of policy; resolved at S2 per §9.2).
3. **A documented Stage-1 compensation is live in `PLAY_TYPE_SHOT_ZONES` — Stage 2 owns the unwind.** The engine's hardcoded play-type *frequencies* run far from Synergy reality (cut and isolation over-selected; transition and pick-and-roll under-selected). Fully-real per-type shot diets under that skewed mix would land league rim share several points high, so cut and spot_up diets are **deliberately shaded** toward short_midrange/threes "to the edge of narratability" — flagged in-file as `KNOWN STAGE 2 ARTIFACT` with an explicit re-tune instruction. The acceptable, documented form of the pool-artifact trap — but debt with a due date (**S2**, §4.2).
...
5. **Resolved in the R0a revision-6 diff:** `docs/TRANSACTIONS_ROADMAP.md` now records Phase 5b implemented and schema v6; Phase 5c remains gated by this roadmap's franchise prerequisites.
...
8. **`shouldDoubleTeam` stale doc comment** in `defense.ts` — still listed under `AGENTS.md` "Known cleanups", still unfixed. **R0b**.
9. **`README.md` project-structure section is partially stale**: omits `src/transactions/`, `src/data/saves/`, the `menu`/`league` pages, and the saves API routes. **R0b**.
...
11. **FT anchor nit:** `FT_LEAGUE_AVG_PCT = 0.781` vs. the enforced empirical 0.7823 — inside tolerance; settle at S2's round-trip re-assertion.

### 1.4 Remaining unknowns (much smaller after the local run)

- **The margin failure's root cause** is undiagnosed — S1-R's first job is diagnosis, not tuning (§4.1a).
- **Whether the assisted sign-structure failure shares that root cause** or is an independent chain-credit artifact — also S1-R's diagnostic scope.
- **The finalized Stage 1 implementation prompt** is still not in the repo (no `docs/prompts/`). R0b commits it as a historical record, annotated with §4.1's divergence log **and the acceptance-failure record**.
...
- **Track S — Simulation & data** (§4): the pipeline-driven engine overhaul. Stage 1 (implemented; **S1-R repair pending**) → Stage 2 (ratings/tendencies derivation from data; retire BallDontLie; unwind the Stage-1 shot-diet shading) → Stage 3 (mechanics enabled by richer data). Strictly sequential within the track.
...
| 1a | **S1 — Stage 1 league calibration** | S | ⚠️ **IMPLEMENTED, ACCEPTANCE FAILING** (§1.0/§4.1) — margin out of band; assisted sign-structure check failing. |
| 0′ | **R0a — discoverability & current-state truth-up** | — | 🟡 **IMPLEMENTED AND VERIFIED IN THIS DIFF.** Adds this roadmap, links it from `AGENTS.md`, and corrects the transaction roadmap's Phase-5b/schema-v6 status. Mark done when revision 6 lands. A red inherited profile does not block docs-only work; unchanged red output still proves zero behavioral drift (§3.4). |
| 1a′ | **S1-Ra/b — seed boundary, then Stage-1 acceptance repair** | S | **IMMEDIATE NEXT BEHAVIORAL WORK.** Two ordered prompts/commits: behavior-preserving seed-boundary repair, then margin diagnosis/tuning plus assisted-structure diagnosis (§4.1a). Precedes everything that assumes a green engine baseline. |
| 0″ | **R0b — post-repair housekeeping** | — | After S1-R. Records the accepted Stage-1 result, codifies the settled seed boundary wording, fixes stale comments, creates `docs/prompts/`, and refreshes README (§3.4). |
| 1b | **F1 — team selection** | F | After R0b (or ∥ with it — F1 touches nothing R0b documents-first about). Small; unblocks every GM-facing feature. |
| 2 | **S2a–d — Stage 2 league builder, ratings, tendencies, re-baseline** | S | Four ordered execution units (§4.2), all requiring the green S1-R baseline. They retire BallDontLie, replace heuristic ratings/tendencies, unwind the Stage-1 shot-diet compensation, re-derive spacing/versatility baselines, and re-pass the profile without hiding mapping defects inside one giant retune. |
| 3 | **F2 — playoffs** ∥ **S3 — Stage 3 mechanics (first tranche)** | F / S | Both unblocked; either order, one at a time against main. F2 makes the championship metric real before the 5c baseline. S3 is a gated menu (§4.3) — one item per prompt. |
...
| 7 | **T-5c — league-balance harness + authoritative baseline** | T | Infrastructure before the feature it judges. Trade-free baseline captured on the post-S2/F4/F5 world. Its sequence-level value-flow metrics are now **normative** for T-6/T-7 (§6.1). |
| 8 | **T-6 — trade AI (CPU valuation)** | T | Valuation calibrated on the *real* (post-S2) pool. Judged by the 5c harness; fuzzer trades at any legal in-season date (F5 shipped). |
...
| 11 | **T-8 — the draft** | T | Needs rookie scale, pick assets, development, prospect valuation, the season cycle. Replaces F4's replacement-level generation as the league's inflow. |
...
|---|---|---|---|---|---|---|---|---|
| **S1** *(implemented)* | ● done (targets derived + transcribed) | ✗ **FAILING** (margin — owned by S1-R) | ○ (report deltas) | ○ | ○ (green in local run) | ○ (0.781 nit — §1.3.11) | n/a yet | n/a yet |
| **S1-R** (repair) | ○ unchanged (no target edits — §4.1a) | ● (this *is* the phase) | ○ (report deltas only; modern-era derivation remains Horizon) | ○ (re-check if game-flow constants move shot context) | ● re-run (margin-affecting constants can interact) | ○ | n/a yet | n/a yet |
...
**R0b — after S1-R, docs/comments only:**
...
### 4.1 S1 — Stage 1: league-level empirical calibration ⚠️ IMPLEMENTED — ACCEPTANCE FAILING (contract record)

*This section records what was built, verified in source, including where execution deliberately diverged from the plan (marked **[DIVERGENCE]**). Status correction (revision 4): the 2026-07-06 local run shows the profile **failing** on average margin and the assisted sign-structure informational check failing — so under this project's rules Stage 1 is implemented but **not accepted**. S1-R (§4.1a) owns the repair. The executable artifacts are `scripts/derive-league-targets.ts`, `docs/LEAGUE_TARGETS.md`, the rewritten `scripts/profile-engine.ts`, and the retuned `src/engine/constants.ts`.*
...
6. **Assisted-shot derivation — join gate passed at 100.000%; the anchoring plan changed. [DIVERGENCE — recorded]** The exact `(gameId, gameEventId)=(gameId, actionNumber)` join ran at 309,532/309,532 against the ≥ 99.9% gate. The spec's plan — per-zone assisted rates anchoring `PLAY_TYPE_PASS_RATE` — was **not** adopted: the NBA scorekeeper assist definition is materially more liberal than the engine's strict chain assist, so raw-rate anchoring would anchor to a different quantity. Shipped resolution: the **enforced** chain anchor is the box **assist total** (26.66/team-game, from `box_advanced` — an independent source, which also fills the parse-cross-check role: description-parsed rates are never load-bearing); per-zone assisted rates (corner three 0.967 ≫ all others) are INFORMATIONAL with the *sign structure* as the sanity check, owner **Stage 2/3**. **Revision 4 status: that sign-structure check is currently failing in engine output** — corner three is not the highest-assisted zone — which means the engine's chain does not concentrate assisted credit where the real game does. Diagnosis belongs to **S1-R** (§4.1a); if the fix requires the assist-definition mapping work, S1-R documents that and the item stays with its S2/S3 owner (it is informational and cannot block acceptance — but it cannot be silently ignored either).
...
**Residuals (owners assigned):** margin acceptance failure + assisted sign-structure diagnosis + engine-seed boundary fix → **S1-R** (§4.1a); the `PLAY_TYPE_SHOT_ZONES` shading unwind → **S2**; the assist-definition mapping → **S2/S3**; the FT anchor nit → **S2**; the play-type frequency skew → **S2**.

### 4.1a S1-Ra/b — Stage 1 acceptance repair (IMMEDIATE NEXT WORK; two ordered units)
...
1. **S2a — identity + league builder.** Build the NBA-personId-keyed league artifact and validation/fallback report without changing the active player pool or engine behavior. Keep the legacy ingest path until the new artifact round-trips and coverage gates pass.
2. **S2b — ratings derivation.** Implement shooting/offense/defense/physical rating mappings plus the statistical-contract report; switch the active pool only after distribution checks pass. Tendencies and shot diets remain unchanged in this unit.
3. **S2c — tendencies + compensation unwind.** Implement real usage/play-type/shot-mix tendencies, decide the assist-definition mapping, and remove the documented Stage-1 shot-diet shading.
4. **S2d — coupled re-baseline and retirement of legacy ingest.** Re-derive spacing/versatility baselines, settle the FT inverse pair, retune the realized profile to PASS, then delete or archive the superseded BDL generation path and resolve `scripts/seed-test.ts`.

No unit may borrow from the next to make its own checks green. S2a and S2b may emit candidate artifacts beside the active pool; only the explicit activation point changes runtime inputs.
...
**The Stage-1 compensation unwind (named duty):** once the derived tendencies bring the league play-type distribution near the Synergy reference, **re-tune `PLAY_TYPE_SHOT_ZONES` back toward the real per-type shot diets** noted in its own comments, and remove or rewrite the `KNOWN STAGE 2 ARTIFACT` block. Evidence the unwind actually happened: the informational play-type table moving into the vicinity of the Synergy shares. If the distribution can't get close enough for real diets to hold the zone shares in band, that's a stop-and-surface (it would mean play-type *selection* mechanics are the binding constraint — a Stage 3/Horizon question).

**Assist-definition decision (inherited from S1 decision 6, informed by S1-R's diagnosis):** before per-zone assisted rates can inform `PLAY_TYPE_PASS_RATE`, choose and document the mapping between the NBA scorekeeper assist and the engine's strict chain assist (a derived discount factor, or a deliberate loosening of the engine definition — engine mechanics belonging to S3.g if chosen). Until then the box assist total remains the only enforced chain anchor. Deciding *not* to use the per-zone rates is legitimate; deciding silently is not.
...
**Consequential re-derivations (per the matrix):** re-run `tsx scripts/calibrate-spacing.ts` and adopt the new `SPACING_*`/`VERSATILITY_*` values; re-verify both A/B tests; **re-establish a passing profile on the new pool** (targets unchanged; realized values will move; the unwind is part of this re-tune); re-assert the FT round trip; refresh `scripts/seed-test.ts`'s role (retire in favor of build-league, or convert to seeded generation per §9.2 — decide, don't drift).
...
- **S3.a Defender-matchup fidelity.** `defense.matchupsByOppPosition` and defended-category FG%± to sharpen `selectDefender` and the defender term in `resolveShot`. Directly deepens the weak-link versatility story.
- **S3.b Lineup-model validation harness.** `tsx scripts/validate-lineups.ts` scoring the spacing+versatility model against real five-man `lineups` net ratings (2007–25). Cheap, high leverage — reasonable to do first; becomes the regression check for every later fit change.
...
### 5.1 F1 — Team selection & controlled-franchise identity (Wave 1b; small)

**Goal:** give the game a persistent answer to "which team is the player?" — the field every GM-facing feature keys on.

**Verified gap:** the gate accepts `controlledTeamId?` per-call for NTC consent (`gate.ts`, `validators.ts`); no field exists on `SeasonState`/`SaveFile`.
...
- Wire the existing gate option: transaction call sites with a save in hand pass the canonical value.
...
The transactions roadmap defines the metrics (talent dispersion / Gini, championship & playoff entropy, churn and value-pump signals), the fixed-seed N-season structure, the machine-diffable summary, and the baseline-first discipline. Five amendments from this roadmap's vantage:
...
4. **Sequence-level value-flow metrics are NORMATIVE (revision 4).** The per-trade referee (§6.2) is a filter, not a proof — cyclic laundering (A→B→C→A) and slow asymmetric bleed are *sequence* phenomena a per-trade check cannot see. The harness therefore computes, per run: **cumulative net base-value flow per team over N seasons** (under the versioned shared valuation model, marked-at-trade-time), **value-bearing cycle detection** over the trade graph, and drift bounds (no team's cumulative net base-value inflow from CPU↔CPU trades exceeds a named tolerance). A cycle is not itself a failure; it fails only when marked-at-trade-time value transported around it exceeds the declared tolerance or exhibits repeatable laundering. These are pass/fail criteria for T-6 and T-7 acceptance — not informational.
5. **Baseline policy binds to the re-baseline matrix (§3.3).** The authoritative trade-free baseline is captured on the post-S2/F4/F5 world, immediately before T-6, and re-captured whenever a ● phase lands. Championship entropy uses the real F2 champion (or the top-seed proxy pre-F2, labeled as such).

### 6.2 T-6 — trade AI / CPU valuation (amendments)

- **Calibrate on the real pool.** T-6 lands post-S2; its valuation calibration pass runs against the pipeline-derived pool, re-run if anything in the matrix fired since.
...
- **Acceptance:** the 5c harness with fuzzer + valuation enabled, within tolerance of the trade-free baseline, **passing the normative sequence metrics** (§6.1.4) over N seasons.
...
The transactions roadmap owns the structure (pick assets on the `TradeAsset[]` seam; protections/swaps/Stepien as predicates; prospect generation as noise over the potential model, through `SeededRNG`; rookie-scale auto-contracts; the draft event as a slot in F3's seam between rollover and free agency). Amendments:

- **Empirical prospect priors.** `combine` (2000–25) and `players` (draftYear/round/pick) joined to `box_advanced` careers yield real **pick-value curves** and measurable-vs-outcome priors, shaping prospect generation and feeding Phase-6 valuation a pick-value input that isn't folklore. `tsx scripts/derive-pick-value.ts`, same discipline as S1/F4 — including the survivor-bias caution (curves computed only on players who logged NBA minutes overstate late-pick value; busts who never played are part of the curve) and F4-style validation.
- **Prospects develop through F4** — no parallel path; scouting uncertainty is fuzz over potential.
- **The draft replaces F4's replacement-level generation** as the league's inflow, demoting it to a final fallback (mirroring the autofill's fate at T-7). The function stays as the safety net; its header gains the demotion note.
...
2. **Seed policy (effective after S1-Ra; known live violation until then).** `Math.random` and ambient time-based seeds are prohibited inside `src/engine`, **without exception** — engine public constructors require a seed. Ambient randomness is sanctioned only at the app boundary (menu/API), choosing a brand-new world's validated seed, which is then persisted; everything downstream is `SeededRNG` on stable keys. Data-shaping/generation scripts take their seed as input (`scripts/seed-test.ts` is out of policy; resolved at S2). **Lineage:** one-shot boundary generation may use save-independent per-id keys (`fnv1a(id)`); *recurring per-season* world-state generation must descend from the world's seed via `deterministicSeed(season.seed, stableKey)`. UI-only display noise (scouted-ratings fuzz) is exempt — it renders, never persists. S1-Ra removes the `createSeasonState` and `simulateSeason` defaults; R0b then codifies the shipped boundary in `AGENTS.md`.
...
| **Profile PASS status** | the engine itself | `npm run profile` (exit code) | **Currently FAILING** (margin 16.8 vs 12.87 ± 1.0, 2026-07-06 run). Owner: **S1-R**. Every subsequent phase's baseline. |
...
| FT inverse pair (`FT_LEAGUE_AVG_PCT`/`FT_PCT_SLOPE`/`FT_DERIVE_SCALE`) | `engine/constants.ts` | Round-trip printed by `scripts/recompute-derived.ts` | Change together, always. Open nit: 0.781 vs. empirical 0.7823 — settle at S2. |
| Spacing/versatility baselines (`SPACING_*`, `VERSATILITY_*`) | `engine/constants.ts` | `tsx scripts/calibrate-spacing.ts` | A/Bs green in the 2026-07-06 run; re-derived at S2; per-season deterministic snapshots at F4 (constants remain the season-1 anchor). |
| A/B expectations | `scripts/test-spacing-ab.ts`, `test-defense-ab.ts` | Re-verify at S1-R (margin constants can interact) and S2 | Material, correctly-signed effects. Green 2026-07-06. |
...
| 5c trade-free baseline + **normative sequence metrics** | committed summary JSON from `scripts/league-balance.ts` | Re-run the harness, fixed seed | Capture post-S2/F4/F5, immediately pre-T-6; re-capture on any ● phase. Sequence metrics (cumulative base-value flow, value-bearing cycle detection, drift bounds) are pass/fail for T-6/T-7 (§6.1.4). |
| Phase-6 valuation calibration (base value + context adjustments) | `src/transactions/value.ts` constants + calibration notes | T-6 calibration pass | On the real (post-S2) pool only; base/context split per §6.2. |
