...
5. **S2d — activation, coupled re-baseline, and legacy-ingest retirement.** This is the **sole activation point**: atomically promote the accepted candidate to the active new-league path, re-derive spacing/versatility baselines, settle the FT inverse pair, retune the realized profile to PASS, then delete or archive the superseded BDL generation path and resolve `scripts/seed-test.ts`. S2d also knowingly retires or regenerates the S2c2 measurement check: `scripts/report-s2c2.ts --check` embeds the default-profile hash and pre-promotion candidate behavior, so activation necessarily invalidates it — regenerate or archive it in the S2d change, never leave it silently red.
