...
> **Status:** living master plan, **revision 8 + S3 execution-plan addendum (2026-07-15), S3.b2 partial-scope acceptance (2026-07-21)**. Save schema **v8**; transactions **Phases 1–5b implemented**; Stage 1 accepted; F1 done; **S2a through S2d landed**; **F2 accepted and merged 2026-07-15**; **S3.a accepted 2026-07-15**; **S3.b1 accepted 2026-07-18**; and **partial S3.b2 accepted 2026-07-21** with derived rim, short-midrange, and shared-3PT influence plus the separately named perimeter-only long-midrange legacy fallback (evidence in `docs/PROJECT_STATUS.md`). The NBA-derived pool/selector/diets remain the sole production path; **S3.c1 is the next simulation unit and F3 is the next independent franchise unit**. Every claim below was verified against source or execution — where documents disagreed, the code won (§1.4).
>
...
**App shell** (`src/app/`): menu (new game / save management), league, roster, season schedule/standings/leaders/playoff bracket and champion view, player detail, single-game sim page; API routes for players, teams, season, sim, saves. New-game team picker (F1) selects the controlled franchise (spectator default). No transaction UI or offseason flow.
...
| 3 | **F2 — playoffs** ✅ merged 2026-07-15 ∥ **S3 — Stage 3 mechanics (first tranche)** | F / S | **F2 accepted and merged; S3.a, S3.b1, and partial S3.b2 accepted. S3.c1 is next.** Continue landing one unit at a time on main; F2 makes the championship metric real before the 5c baseline. |
...
5. **S2d — activation, coupled re-baseline, and legacy-ingest retirement. ✅ DONE (2026-07-14); correction candidate verified, final review pending.** The sole activation point deterministically promotes the NBA-derived pair into `data/` through a journaled, self-healing two-rename protocol and records an atomic promotion manifest (pair SHA-256s + production selector/table identities). The correction candidate atop `db5e27e` centralizes the runtime proof in `src/lib/production-pool.ts`: new-game creation and activation-context checks read the exact pair once, validate complete roster/rotation/**minute-target** structure, require NBA-derived IDs, hash-match the builder-owned manifest, and require the sole selector/table identities. The builder no longer executes retired heuristic ratings/tendency derivation before its NBA-derived passes. The NBA-derived selector and sole `PLAY_TYPE_SHOT_ZONES` table are the production path; the old alternate-pool/table configuration and retired ingest/seed/report generators are not runtime interfaces. Spacing AND versatility baselines are re-derived by `scripts/calibrate-spacing.ts` and transcribed; the FT inverse pair is settled on the empirical 2023-26 FTA-weighted anchor with the round trip asserted in the S2d harness. The coupled retune landed profile **PASS 32/32** on the activated pool: the former global three-point dampener folded into the annotated per-zone `SHOT_ZONE_FREQUENCY_FACTORS`, plus steal-split / block / foul-rate / pass-rate knobs (all named constants). The active terminal-event selector contract holds on all three predeclared seeds (4.55/4.43/4.28 pp within the predeclared 6.00pp band); it includes makes, misses, and-ones, and turnovers, rather than downstream field-goal attempts only. The correction preserved profile/calibrate byte identity, determinism, and spacing A/B evidence; exact hashes and commands are in `docs/PROJECT_STATUS.md`. The deleted-generator reports are frozen historical evidence, not current generators; their measurements remain in those artifacts while status lives in this roadmap and `PROJECT_STATUS.md`. Target SDs are frozen with provenance as `S2B_TARGET_SDS` (`src/ratings/nba-derivation.ts`) — deliberate compatibility priors, no longer derivable from the retired pool. Existing saves retain their persisted players.
...
**Consequential re-derivations (S2d, per the matrix):** re-run `tsx scripts/calibrate-spacing.ts` and adopt the new `SPACING_*`/`VERSATILITY_*` values; re-verify both A/B tests; **re-establish a passing profile on the activated pool** (targets unchanged; realized values will move; the unwind is part of this re-tune); and re-assert the FT round trip. The former legacy seed generator was retired at activation; it has no follow-up role.
...
**S3.a — historical lineup-model validation. ✅ accepted 2026-07-15.** Prompt: `docs/prompts/S3A_LINEUP_VALIDATION_IMPLEMENTATION_PROMPT.md`.
...
**Accepted outcome:** the 2017-18 through 2024-25 partial-possession sample produced a generated, supply-adjusted 5×3 lift table; runtime selection now uses one explained weight vector and exactly one unconditional RNG draw. The focused one-draw/lift/hunt/reachability harness, lineup oracle, determinism, spacing and defense A/B checks are green, and the activated-pool profile remains PASS 32/32. Exact coverage, deltas, and hashes live in `docs/PROJECT_STATUS.md`; partial S3.b2 followed.

**S3.b2 — zone-specific defender influence. ✅ partial scope accepted 2026-07-21.** Prompt: `docs/prompts/S3B2_DEFENDER_INFLUENCE_IMPLEMENTATION_PROMPT.md`.

Use defended-category FG%± to derive the existing perimeter/interior-defense blend by shot zone. Change only `getDefenderRating` / the current defender term in `resolveShot`; do not add a second defense modifier or change defender selection. A raw 40/40 defender has zero defender modifier at zero fatigue, and S3.a's lineup score may not regress.

**Authorized partial scope — 2026-07-21 diagnostic decision.** The predeclared `2013-14` through `2024-25` full-window fit remains primary. It supports derived rim, short-midrange, and shared-3PT weights; the short-midrange full-window interior weight is approximately `0.590`, with the predeclared early/late movement (`0.738` / `0.461`) retained as disclosed sensitivity rather than alternate runtime modes. Long two did not clear the unchanged player-clustered 95% slope-sum gate: its confidence interval includes zero in the full, early, and late fits. Therefore `long_midrange` preserves the accepted perimeter-only behavior (`interiorWeight = 0`) through a separately named **legacy fallback**, not a derived weight. The implementation must still fit and report long two, may not change the confidence level or filters, and may not claim identification. Read-only diagnostic stdout SHA-256: `c147250bf80c745f700947f92f1585643d8d2cae6ea0fb00269cdd7b47cc448d`.

**Accepted outcome:** the deterministic full-window derivation published interior weights `0.933818` for rim, `0.589576` for short midrange, and `0.000000` for the shared 3PT family. Runtime applies the zone blend once, fatigue once, and the existing centered defender modifier once. `long_midrange` remains `S3B2_LONG_MIDRANGE_LEGACY_INTERIOR_WEIGHT = 0`, explicitly a legacy fallback; the generated report continues to disclose its full/early/late zero-crossing intervals and does not claim identification. No new RNG draw or compensating engine adjustment was introduced. The focused derivation, equivalence, centering, fatigue-order, RNG-trace, monotonicity, and clamp harness is green; the activated-pool profile remains PASS 32/32. Exact deltas and hashes live in `docs/PROJECT_STATUS.md`; S3.c1 is next.
...
### 5.2 F2 — Playoffs ✅ ACCEPTED LOCALLY 2026-07-14 (Wave 3)

> **Outcome record.** F2 shipped the named modern-format constants, pure tie-group
> seeding, bracket-position game IDs, play-in through Finals, separate postseason
> stats/results, shared monotonic advancement/injury machinery, schema-v8 migration,
> bracket UI, and champion metadata. The live regular schedule is 1,290 games (86 per
> team), not the stale 1,230 example below; F2 freezes at `totalGames` and deliberately
> leaves that schedule byte-identical. Acceptance evidence is in `PROJECT_STATUS.md`.
...
- **Stats separation:** `playoffPlayerStats` separate; regular-season stats and standings freeze at the persisted regular `totalGames` boundary (currently 1,290 games).
...
8. **Schema-version ledger.** v8 = current (F2 persisted playoffs; v7 = F1 controlled-franchise identity; v6 = usage/FT recompute). Planned: v9 F3 (careerStats provenance + season counter) · v10 F4 (per-season baselines, retirement entries) · v11 F5 (stat stints) — indicative, renumber freely as phases land, but **every** persisted-state phase bumps, migrates, and round-trips, and the `models/save.ts` docblock ledger stays the canonical history.
...
| **Historical lineup-model validation oracle** | `docs/S3_LINEUP_VALIDATION.md` + frozen input `docs/S3_LINEUP_VALIDATION_BASELINE.json` | `node --import tsx scripts/validate-lineups.ts` (`--check` for byte identity and regression enforcement) | **S3.a.** Deterministic season-as-of spacing/versatility validation over completed five-man lineup outcomes, with contract-era fallback rates, out-of-fold LOSO calibration errors, frozen per-comparison tolerance, and clamp-saturation diagnostics. The report is generated measurements; the baseline is updated only by an explicit acceptance decision. |
| **Defender-matchup lift matrix** | generated block in `src/engine/constants.ts` + `docs/S3B1_MATCHUP_DERIVATION.md` | `node --import tsx scripts/derive-s3b1-matchups.ts` (`--check` byte-compares both outputs) | **S3.b1.** Partial-possession-weighted 2017-18 through 2024-25 defender-position × coarse opponent-position lift, supply-adjusted within the production position vocabulary. The report is generated measurements/provenance; runtime reads only the committed constants. |
| **Defender-zone influence mapping** | generated block in `src/engine/constants.ts` + `docs/S3B2_DEFENDER_INFLUENCE.md` | `node --import tsx scripts/derive-s3b2-defender-influence.ts` (`--check` byte-compares both outputs) | **Partial S3.b2 accepted 2026-07-21.** Full-window derived rim (`0.933818`), short-midrange (`0.589576`), and shared-3PT (`0.000000`) weights are live. `long_midrange` remains the separately named perimeter-only legacy fallback (`interiorWeight = 0`) because long two's unchanged 95% slope-sum gate includes zero in the full and both half-windows; it is measured, not identified. Decision diagnostic SHA-256 `c147250bf80c745f700947f92f1585643d8d2cae6ea0fb00269cdd7b47cc448d`. |
