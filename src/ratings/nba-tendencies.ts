...
import { FULL_CONFIDENCE_SAMPLE, RECENT_SEASON_WEIGHTS } from './nba-derivation';
...
export interface TendencyResult { tendencies:Map<number,PlayerTendencies>; fallbackLog:{playerId:string;field:string;reason:string}[]; coveredSynergy:number; coveredShots:number; coveredUsage:number; synergyPossessions:Map<number,number>; shotFga:Map<number,number>; }
...
export function deriveNbaTendencies(input: readonly TendencyInput[], playtypes: readonly PlayTypeRow[], shots: readonly ShotEventRow[]): TendencyResult {
 ...
 return {tendencies,fallbackLog:fallbackLog.sort((a,b)=>a.playerId.localeCompare(b.playerId)||a.field.localeCompare(b.field)),coveredSynergy,coveredShots,coveredUsage,synergyPossessions,shotFga};
}
