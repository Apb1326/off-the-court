...
- **Controlled identity is read through the accessors; the gate stays symmetric.** The canonical controlled-team identity is top-level `SaveFile.controlledTeamId`, read via `getControlledTeamId`/`isControlledTeam` (`src/franchise/controlled.ts`) — never ad-hoc field access. The legality gate and validators never branch on controlled status; the existing per-call `controlledTeamId` option (NTC consent) remains the only transaction seam.
