...
const RUNTIME_BUCKETS: readonly RuntimeMatchupBucket[] = ['G', 'F', 'C'];
...
interface Derivation {
  ...
  aggregated: Matrix<RuntimeMatchupBucket>;
  columnShares: Matrix<RuntimeMatchupBucket>;
  lifts: Matrix<RuntimeMatchupBucket>;
  positionMarginal: Record<Position, number>;
  bucketMarginal: Record<RuntimeMatchupBucket, number>;
...
function assertAggregationVocabulary(): void {
  const locked: Record<RawBucket, RuntimeMatchupBucket> = {
...
function derive(): Derivation {
  ...
  ])) as Record<RuntimeMatchupBucket, number>;
  ...
  const bucketMarginal = Object.fromEntries(RUNTIME_BUCKETS.map((bucket) => [bucket, bucketTotals[bucket] / knownPartialPoss])) as Record<RuntimeMatchupBucket, number>;
...
function renderConstantBlock(result: Derivation): string {
  ...
  const lines = [
    ...
    'export const S3B1_MATCHUP_LIFT: Readonly<Record<Position, Readonly<Record<RuntimeMatchupBucket, number>>>> = {',
  ];
  for (const position of DEFENDER_POSITIONS) {
