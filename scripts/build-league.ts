...
function buildLeague(): BuildResult {
  ...
  const tendenciesReport = renderTendenciesContract(tendencyDerivation, built, rosteredPlayers.length, rotationPlayers.length);
...
function renderTendenciesContract(result: ReturnType<typeof deriveNbaTendencies>, built: BuiltPlayer[], rostered: number, rotation: number): string {
  const eligible = built.length;
  const average = (field: keyof Player['tendencies']) => built.reduce((sum, bp) => sum + bp.player.tendencies[field], 0) / built.length;
  ...
  out.push(`| pick_and_roll (both fields) | ${(average('pickAndRollBallHandlerFreq') + average('pickAndRollScreenerFreq')).toFixed(4)} | ${playTargets.pickAndRoll.toFixed(4)} | ${(average('pickAndRollBallHandlerFreq') + average('pickAndRollScreenerFreq') - playTargets.pickAndRoll).toFixed(4)} |`);
  for (const [label, field] of plays) { const target=playTargets[field]; const value=average(field); out.push(`| ${label} | ${value.toFixed(4)} | ${target.toFixed(4)} | ${(value-target).toFixed(4)} |`); }
  out.push('', '| Shot bucket | Candidate share | Target | Delta |', '| --- | ---: | ---: | ---: |');
  for (const field of ['rimRate','midrangeRate','threePointRate'] as const) { const value=average(field); out.push(`| ${field} | ${value.toFixed(4)} | ${shotTargets[field].toFixed(4)} | ${(value-shotTargets[field]).toFixed(4)} |`); }
